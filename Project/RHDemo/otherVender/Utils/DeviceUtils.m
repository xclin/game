...
#define SDKIdentifier [NSString stringWithFormat:@"01_%@_%@_20200420_sdkxy",[[NSBundle mainBundle] bundleIdentifier],[sdkInitConfiger share].ver]
...
+ (NSString *)idfa {
    NSString *idfaStr = [COMMONMETHOD getEquipmentIDFA];
    return idfaStr;
}
...
+ (NSString *)deviceMsg {
    ...
    NSString *version = [DeviceUtils sysVersion]; //系统版本
    NSString *resolution = [NSString stringWithFormat:@"%.0f*%.0f",[self getWidth],[self getHeight]];
    NSData * jsonData = [NSJSONSerialization dataWithJSONObject:@{@"phone_ip":ip, @"phone_model":model, @"phone_version":version,@"phone_resolution":resolution,@"idfa":[COMMONMETHOD getEquipmentIDFA],@"ver":[sdkInitConfiger share].ver,@"ver_name":[sdkInitConfiger share].verName,@"idfv":getIDFV,@"batch_no":SDKIdentifier,@"network_type":[self networkType],@"network_name":network_name,@"progress":[NSProcessInfo processInfo].processName,@"behavior_id":@""} options:0 error:nil];
...
+ (NSString *)devicetoken {
    ...
    NSString *string = [KeyChainUtils getKeyChainWithkey:@"SDKDEVICEID"];
    if (string.length > 0) {
        deviceID = string;
    } else {
        ...
        [KeyChainUtils saveKeyChainWithString:timeLocal.md5String Andkey:@"SDKDEVICEID"];
    }
    return deviceID;
}
...
+ (NSString *)generateSignKey:(NSString *) keyString {
    if (keyString) {
        NSString * signKey = [keyString stringByAppendingFormat:@"%@",[sdkInitConfiger share].privateKey];
...
+ (NSString *)cpsid {
    NSString *cpsid = @"0";
    NSDictionary *dic = [DeviceUtils getGPSIDConfig];
    if (dic) {
