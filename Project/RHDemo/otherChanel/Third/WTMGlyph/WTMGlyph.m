...
- (id)initWithName:(NSString *)_name JSONObject:(NSArray *) jsonObject {
    if (!(self = [self init])) return nil;
    self.name = _name;
    [self createTemplatesFromJSONObject:jsonObject];
    return self;
}
...
- (void)createTemplatesFromJSONObject:(NSArray *)arr {
    int i = 0;
    for (NSString * pointString in arr) {
        NSArray * points = [pointString componentsSeparatedByString:@" "];
        WTMGlyphStroke *stroke = [[WTMGlyphStroke alloc] init];
        for (int i = 0; i < points.count / 2; i ++)[stroke addPoint:CGPointMake([points[i*2] floatValue], [points[i*2+1] floatValue])];
    ...
    if (count == 1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (count % 2 == 1) {
                ...
                [strokeOrders replaceObjectAtIndex:0 withObject:@(last)];
                [strokeOrders replaceObjectAtIndex:(count-1) withObject:@(first)];
            } else {   
                int next = [[strokeOrders objectAtIndex:i] intValue];
                [strokeOrders replaceObjectAtIndex:i withObject:@(last)];
                [strokeOrders replaceObjectAtIndex:(count-1) withObject:@(next)];
            }
        }
    }
}
